...
class CalendarController {
  ...
  async setNextMeeting(refreshToken, meetingDetails) {
    ...
    const availability = await this.calendarService.getTimesBusy(
      refreshToken,
      calendars
    );
    const meetingTime = this.findFirstAvailableSlot(
      availability,
      meetingDetails.lengthMinutes
    );
  ...
  async setNextMeetingGroup(currentUserRefreshToken, groupId, meetingDetails) {
    ...
    for (let member of group.members) {
      let calendars = await this.calendarService.getCalendars(
        member.refreshToken
      );
      let memberAvailability = await this.calendarService.getTimesBusy(
        member.refreshToken,
        calendars
      );
      availability = availability.concat(memberAvailability);
    }
    const emails = group.members.map(function (member) {
  ...
  findFirstAvailableSlot(availability, meetingLengthMinutes) {
    try {
      ...
      if (
        ...
        proposedMeetingStart.isBetween(mergedTimes[0][0], mergedTimes[0][1])
      ) {
      ...
      while (moment(proposedMeetingStart) < moment().add(14, 'days')) {
        let maxTime =
          mergedTimes.length > counter
            ? new Date(mergedTimes[counter][0])
            : moment().add(14, 'days');
        let meetingTime = this.findAvailability(
          moment(proposedMeetingStart),
          maxTime,
    ...
    try {
      while (proposedMeetingStart < maxTime) {
        if (this.isTimeBetweenBounds(proposedMeetingStart, minTime, endTime)) {
          let proposedMeetingEnd = this.addMinutesToDate(
            proposedMeetingStart,
            meetingLengthMinutes
          );
          if (
            proposedMeetingEnd <= maxTime &&
            this.isTimeBetweenBounds(proposedMeetingEnd, minTime, endTime)
          ) {
  ...
  mergeOverlappingSlots(availability) {
    ...
    for (let i = 1; i < availability.length; i += 1) {
      if (previousEvent[1] >= availability[i].start) {
        let endDate =
          previousEvent[1] >= availability[i].end
            ? previousEvent[1]
            : availability[i].end;
  ...
  shiftTimeToBound(proposedStart) {
    ...
    let duration = moment
      .duration(startOfMeetingTimes.diff(proposedMeetingStart))
      .asHours();

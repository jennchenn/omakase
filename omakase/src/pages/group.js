...
function Group() {
  ...
  const createEvent = async (event) => {
    ...
    if (res.status === 200) {
      alert(
        `Created meeting: ${res.data.start.dateTime} - ${res.data.end.dateTime}`
      );
    }
  };
    ...
    <div style={pageStyles}>
      ...
      <Form style={buttonStyle} onSubmit={createEvent}>
        <Form.Group controlId="eventName">
          ...
          <Form.Control
            placeholder={`${groupInfo.name} Event`}
        ...
        <Form.Group controlId="eventDescription">
          ...
          <Form.Control
            placeholder="event, made with omakase!"
      ...
      <div style={buttonStyle}>
        <Button style={bodyStyle} variant="dark" href="/dashboard">
          ← dashboard
        </Button>
      </div>
    </div>
